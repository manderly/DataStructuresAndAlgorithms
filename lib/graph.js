...
Graph.prototype.bfs = function(source) {
  ...
  while (queue.length > 0) {
    var v = queue.shift(); //v is whatever was just removed from the front of the queue

    if (v !== undefined) {
      console.log('Visited vertex: ' + v); //offby1: starts at 0
    }
...
Graph.prototype.topSortHelper = function(v, visited, stack) {
  //when a vertex is passed in, mark it as visited
  visited[v] = true;
  for (var adjvertex in this.adjacencies[v]) {
    if (!visited[adjvertex]) {
      //re-run function on adjacent vertices to this one
      this.topSortHelper(visited[adjvertex], visited, stack);
    }
  }
  console.log('about to push v ' + v);
...
Graph.prototype.pathTo = function(source, v) { //source and vertex come in from program file
  ...
  for (var i = v; i != source; i = this.edgeTo[i]) { //set i = to v, and push every path to the array
    console.log("shortestpath.push " + i); //push path (or edge)
    shortestpath.push(i);
  }
